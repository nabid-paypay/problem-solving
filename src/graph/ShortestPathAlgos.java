...
public class ShortestPathAlgos {
    ...
    private void e_Neighbours(int u) {
        ...
        for (int i = 0; i < adj.get(u).size(); i++) {
            ...
            if (!settled.contains(v.data)) {
                ...
                pq.add(new Node(v.data, dist[v.data]));
            }
        }
    }
    ...
    /*public int findCheapestPrice(int n, int[][] flights, int src, int dst, int K) {
        ...
        for (int[] array : flights){
            adj.get(array[0]).add(new Node(array[1], array[2]));
        }
        int[] dist = new int[n];
        for (int i = 0; i <n ; i++) {
            dist[i] = Integer.MAX_VALUE;
        ...
        while (settled.size()!=n && !pq.isEmpty()){
            int u = pq.remove().data;
            settled.add(u);
            exploreNei(u, adj, settled, dist, pq);
        }
    ...
    private void exploreNei(int u, List<List<Node>> adj, Set<Integer> settled, int[] dist,
                            PriorityQueue<Node> pq) {
        for (int i = 0; i < adj.get(u).size(); i++) {
            Node v = adj.get(u).get(i);
            if (!settled.contains(v.data)) {
                int edgeDist = v.cost;
                int newDist = dist[u] + edgeDist;
                if (newDist < dist[v.data]) {
                    dist[v.data] = newDist;
                }
                pq.add(new Node(v.data, dist[v.data]));
            }
    ...
    public int findCheapestPrice(int n, int[][] flights, int src, int dst, int K) {
        ...
        for (int[] array : flights) {
            adj.putIfAbsent(array[0], new ArrayList<>());
            adj.get(array[0]).add(new int[]{array[1], array[2]});
        }
        dfs(adj, src, dst, K + 1, 0);
        return totalCost == Integer.MAX_VALUE ? -1 : totalCost;
    }

    private void dfs(Map<Integer, List<int[]>> adj, int src, int dst, int k, int cost) {
        if (k < 0) {
            return;
        }
        if (src == dst) {
        ...
        for (int[] node : adj.get(src)) {
            if (cost + node[1] > totalCost) continue;
            dfs(adj, node[0], dst, k - 1, cost + node[1]);
        }
    }
    ...
    public int findCheapestPriceBFS(int n, int[][] flights, int src, int dst, int K) {
        ...
        while (!queue.isEmpty()) {
            ...
            for (int i = 0; i < size; i++) {
                int[] curr = queue.poll();
                if (curr[0] == dst) totalCost = Math.min(totalCost, curr[1]);

                if (!adj.containsKey(curr[0])) continue;
                for (int[] array : adj.get(curr[0])) {
                    if (array[1] + curr[1] > totalCost) continue;
                    queue.add(new int[]{array[0], array[1] + curr[1]});
                }
            }
    ...
    public int findCheapestPricePriorityQueue(int n, int[][] flights, int src, int dst, int K) {
        Map<Integer, List<int[]>> adj = new HashMap<>();
        ...
        PriorityQueue<int[]> pq = new PriorityQueue<int[]>(Comparator.comparingInt(elem -> elem[1]));
        ...
        while (!pq.isEmpty()) {
            ...
            if (stop > 0) {
                ...
                for (int[] array : adj.get(node)) {
                    pq.add(new int[]{array[0], cost + array[1], stop - 1});
                }
            }
        }
        return -1;
    ...
    public int networkDelayTimeDFS(int[][] times, int N, int K) {
        ...
        for (int node : adj.keySet()) {
            Collections.sort(adj.get(node), Comparator.comparingInt(elem -> elem[1]));
        }
        int[] dist = new int[N + 1];
        for (int i = 1; i <= N; i++) {
            dist[i] = Integer.MAX_VALUE;
        }
        networkDfs(adj, K, 0, dist);
    ...
    private void networkDfs(Map<Integer, List<int[]>> adj, int src, int cost, int[] dist) {
        if (cost >= dist[src]) return;
        dist[src] = cost;
        if (adj.containsKey(src)) {
            for (int i = 0; i < adj.get(src).size(); i++) {
                int[] node = adj.get(src).get(i);
                //dist[src] = cost + node[1];
                networkDfs(adj, node[0], cost + node[1], dist);
            }
        }
    }

    public int networkDelayTimeDIJ(int[][] times, int N, int K) {
    ...
    public int networkDelayTime(int[][] times, int N, int K) {
        ...
        dist[K] = 0;
        // PriorityQueue<int[]> pq = new PriorityQueue<>(Comparator.comparingInt(elem -> elem[1]));
        PriorityQueue<int[]> pq = new PriorityQueue<>((elem1, elem2) -> elem1[1] - elem2[1]);
        pq.add(new int[]{K, 0});

        while (settled.size() != N && !pq.isEmpty()) {
            int[] curr = pq.remove();
            if (settled.contains(curr[0])) continue;
            settled.add(curr[0]);
            explore(adj, curr, pq, dist, settled);
        }
        int max = Integer.MIN_VALUE;
        for (int i = 1; i <= N; i++) {
            if (dist[i] == Integer.MAX_VALUE) return -1;
    ...
    private void explore(Map<Integer, List<int[]>> adj, int[] curr, PriorityQueue<int[]> pq,
                         int[] dist, Set<Integer> settled) {
        if (adj.containsKey(curr[0])) {
            for (int i = 0; i < adj.get(curr[0]).size(); i++) {
                int[] node = adj.get(curr[0]).get(i);
                if (!settled.contains(node[0])) {
                    if (node[1] + dist[curr[0]] < dist[node[0]]) {
                        dist[node[0]] = node[1] + dist[curr[0]];
                    }
    ...
    public int networkDelayTimeBellmanFord(int[][] times, int N, int K) {
        ...
        for (int i = 0; i < N; i++) {
            for (int[] edge : times) {
                ...
                int desti = edge[1];
                int cost = edge[2];
                if (dist[src] != Integer.MAX_VALUE && dist[desti] > dist[src] + cost) {
                    dist[desti] = dist[src] + cost;
                }
            }
        }
            ...
            for(int i=0; i<size; i++){
                ...
                for(int[] dir : dirs){
                    ...
                    if(nx>=0 && nx<rows && ny>=0 && ny<cols &&
                        ...
                        grid[nx][ny] = Math.min(grid[nx][ny], level);
                        q.add(new int[]{nx, ny});
                    }
                }
            }
    ...
    public static void main(String[] args) {
        ...
        ShortestPathAlgos dpq = new ShortestPathAlgos();
